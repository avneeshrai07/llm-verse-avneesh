...
async def amazon_nova2_lite_function(request: LLMRequest) -> dict:
    """Amazon Nova 2 Lite via Bedrock. Plain text or structured (pydantic_model) output; pass tools=[...] to run an agentic tool-calling loop."""
    ...
    if request.pydantic_model is not None:
        ...
        if parsed is None:
            ...
            logger.error(
                "nova-2-lite [structured] | identifier=%s | failed to parse into %s: %s",
                request.llm_identifier, request.pydantic_model.__name__, parsing_error,
            )
            raise ValueError(
                f"nova-2-lite returned a response that could not be parsed into "
    ...
    # registration/model id for a "nova-2-lite-agentic" variant.
    if request.tools:
    ...
    for iteration in range(max_iterations):
        ...
        logger.info(
            "nova-2-lite | identifier=%s | iteration=%d/%d | %d tool call(s): %s",
            ...
            for tool in request.tools or []:
                if getattr(tool, "name", None) == tool_name:
                    try:
                        ...
                        logger.error("nova-2-lite | tool error | %s | %s", tool_name, exc)
