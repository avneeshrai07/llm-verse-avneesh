...
async def amazon_nova_lite_function(request: LLMRequest) -> dict:
    ...
    # registration/model id for a "nova-lite-agentic" variant.
    if request.tools:
    ...
    for iteration in range(max_iterations):
        ...
        logger.info(
            "nova-lite | identifier=%s | iteration=%d/%d | %d tool call(s): %s",
            ...
            for tool in request.tools or []:
                if getattr(tool, "name", None) == tool_name:
                    try:
                        ...
                        logger.error("nova-lite | tool error | %s | %s", tool_name, exc)
