...
SUPPORTS_TOOLS = True
NOTES = "OpenAI's open-weight reasoning model, served by Groq. Text-only - no vision support."


@register("gpt-oss-120b")
async def gpt_oss_120b_function(request: LLMRequest) -> dict:
    """GPT OSS 120B via Groq. Plain text or structured (pydantic_model) output; pass tools=[...] to run an agentic tool-calling loop."""
    ...
    if request.pydantic_model is not None:
        ...
        logger.info("gpt-oss-120b [structured] | identifier=%s | input=%d | output=%d",
    ...
    # registration/model id for a "gpt-oss-120b-agentic" variant.
    if request.tools:
    ...
    for iteration in range(max_iterations):
        ...
        logger.info(
            "gpt-oss-120b | identifier=%s | iteration=%d/%d | %d tool call(s): %s",
            ...
            for tool in request.tools or []:
                if getattr(tool, "name", None) == tool_name:
                    try:
                        ...
                        logger.error("gpt-oss-120b | tool error | %s | %s", tool_name, exc)
    ...
    logger.info("gpt-oss-120b [text] | identifier=%s | input=%d | output=%d",
